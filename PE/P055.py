...
# has managed so far to map it to a palindrome. In fact, 10677
...
#reverse = int(reverse(string[:(len(string)-1)//2])
def isPalindrome(num):
    num = str(num)
    mid = (len(num))//2
    str1 = num[:mid]
    str2 = num[len(num)-mid:]
    str2 = str2[::-1]
    return str1 == str2

def isLychrel(startNum):
    for i in range(50):
        reverse = int(str(startNum)[::-1])
        startNum+=reverse
